  ...
  render() {
    ...
    setKids(this.el,
      h('div', { class: 'card-head' },
        ...
        h('button', { class: 'btn sm primary', onClick: () => this.openCreator() }, icon('plus'), 'New mission')),
      list.length ? h('div', { class: 'mission-grid' }, ...list.map((m) => this.card(m)))
        : h('div', { class: 'empty' }, art('journey'), h('p', { class: 'muted italic small' }, 'No missions yet — create one with the button above.')));
  },
...
const Calendar = {
  ...
  blockEl(s, sg, interactive = true) {
    ...
    if (isTournament(s)) {
      ...
      return h('div', { class: 'blk tourn', role: 'gridcell', tabindex: interactive ? '0' : null, title: t, 'aria-label': t, dataset: { id: s.id, day: sg.day },
        ...
        h('span', { class: 't' }, icon('flag'), name), h('span', { class: 'when' }, fmtRange(tm.start, tm.end)), h('span', { class: 'lock' }, icon('check-outline')));
    }
  ...
  onKey(e, s) {
    ...
    if (sessionLocked(s) && e.key !== 'Enter' && e.key !== ' ') { e.preventDefault(); toast('That day is closed — reopen it in the day panel', { error: true }); return; }
    ...
    const errs = validateSession({ ...s, ...patch }, s.id);
    if (errs.length) {
      const moves = planPush(start, end, s.id, e.key === 'ArrowLeft' ? -1 : 1);
      if (moves && moves.length) { applyBatch([{ id: s.id, patch }, ...pushMoves(moves)]); this.focusId = s.id; toastUndo(`Block adjusted · ${moves.length} neighbour${moves.length > 1 ? 's' : ''} shifted`); return; }
      toast(errs[0].message, { error: true }); return;
    }
  ...
  fitMove(start, end, excludeId) {
    ...
    for (const [s2, e2] of cands) if (!findOverlap(s2, e2, excludeId)) return { start: s2, end: e2, ok: true, clamped: true, raw: { start, end } };
    return { start, end, ok: false, raw: { start, end } };
  },
  fitEnd(start, end, excludeId) { const snap = Math.max(5, state.settings.snap_min || 15) * MIN; const c = findOverlap(start, end, excludeId); if (!c) return { start, end, ok: true, raw: { start, end } }; const cS = ms(c.started_at); if (cS - start >= snap) return { start, end: cS, ok: true, clamped: true, raw: { start, end } }; return { start, end, ok: false, raw: { start, end } }; },
  fitStart(start, end, excludeId) { const snap = Math.max(5, state.settings.snap_min || 15) * MIN; const c = findOverlap(start, end, excludeId); if (!c) return { start, end, ok: true, raw: { start, end } }; const cE = c.ended_at ? ms(c.ended_at) : nowMs(); if (end - cE >= snap) return { start: cE, end, ok: true, clamped: true, raw: { start, end } }; return { start, end, ok: false, raw: { start, end } }; },

  /* The exact minute matters less than the amount of time: shove the neighbours aside when there is room. */
  tryPush(s, d, res) {
    const want = d.wanted || res.raw; if (!want) return false;
    const dir = d.kind === 'resize-start' ? -1 : d.kind === 'resize-end' ? 1 : (want.start >= d.orig.start ? 1 : -1);
    const moves = planPush(want.start, want.end, s.id, dir);
    if (!moves || !moves.length) return false;
    const shift = want.start - d.orig.start;
    const patch = { started_at: iso(want.start), ended_at: iso(want.end) };
    patch.pauses = d.kind === 'move'
      ? (s.pauses || []).map((p) => ({ start: iso(ms(p.start) + shift), end: p.end ? iso(ms(p.end) + shift) : null }))
      : (s.pauses || []).map((p) => ({ start: iso(clamp(ms(p.start), want.start, want.end)), end: p.end ? iso(clamp(ms(p.end), want.start, want.end)) : null })).filter((p) => !p.end || ms(p.end) > ms(p.start));
    applyBatch([{ id: s.id, patch }, ...pushMoves(moves)]);
    this.focusId = s.id; this.selectedId = s.id;
    toastUndo(`Block adjusted · ${moves.length} neighbour${moves.length > 1 ? 's' : ''} shifted`);
    this.offerMerge(s.id);
    return true;
  },
  /* Same theme, right next door: offer to join them into a single block. */
  offerMerge(id) {
    const cand = mergeCandidate(id, 30); if (!cand) return;
    const other = cand.other; const tm = sessionTimes(other);
    toast(`Another ${themeName(other.theme)} block at ${fmtRange(tm.start, tm.end)} — merge them?`, {
      duration: 9000, action: 'Merge', onAction: () => { const m = mergeSessions(id, other.id); if (m) { toastUndo('Blocks merged'); this.render(); Panel.editSession(m.id); } },
    });
  },
  attachPointer(grid) {
    ...
    const finish = (commitIt, x, y) => {
      ...
      if (res.clamped || d.wanted) { const done = this.tryPush(s, d, res); if (done) return; }
    ...
    grid.addEventListener('pointermove', (e) => {
      ...
      const res = compute(e.clientX, e.clientY); if (res) { drag.wanted = res.raw || null; paintGhost(res.start, res.end, !res.ok, drag.session); }
    });
...
const ReportsCard = {
  ...
  render() {
    ...
    setKids(this.el,
      h('div', { class: 'card-head' }, h('h2', null, 'Reports'), h('div', { class: 'row' }, seg, h('a', { class: 'btn sm', href: '#/week' }, icon('bar-chart'), 'Weekly review'), h('a', { class: 'btn sm', href: '#/report' }, icon('chart'), 'Full report'))),
      h('div', { class: 'stats' },
