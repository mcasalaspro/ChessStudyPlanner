...
const App = {
  ...
  renderMobileNav() {
    ...
    const item = (href, name, label, active) => h('a', { class: 'mnav-item' + (active ? ' on' : ''), href }, icon(name, 'big'), h('span', null, label));
    setKids(nav,
      item('#/', 'calendar', 'Today', page === 'main'),
      item('#/week', 'bar-chart', 'Week', hash.startsWith('week')),
      h('button', { class: 'mnav-cta', onClick: () => StudyNow.open() }, icon('play'), 'STUDY'),
      item('#/report', 'chart', 'Report', page === 'report'),
      item('#/achievements', 'award', 'More', hash.startsWith('achievements')));
  },
