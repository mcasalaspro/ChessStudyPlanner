...
function openModal({ title, body, footer, cls = '', onClose }) {
  ...
  const modal = h('div', { class: 'modal ' + cls }, title ? h('div', { class: 'modal-head' }, h('h2', null, title), h('button', { class: 'btn icon ghost', 'aria-label': 'Close', onClick: () => close() }, icon('close'))) : null, body, footer ? h('div', { class: 'modal-foot' }, footer) : null);
  root.append(modal);
  let closed = false;
...
function showBanner(id, { text, warn, actions = [] }) {
  ...
  const el = h('div', { class: 'banner' + (warn ? ' warn' : '') + (id === 'quote' ? ' quote' : ''), role: 'alert' }, h('div', { class: 'grow' }, text),
    ...
    h('button', { class: 'btn ghost icon sm', 'aria-label': 'Dismiss', onClick: () => hideBanner(id) }, icon('close')));
...
function noteIcon(cls = '') { const el = icon('note', 'note-ic ' + cls); el.title = 'Has a note'; el.setAttribute('aria-label', 'Has a note'); return el; }
...
const Drawer = {
  el: null, current: null,
    ...
    this.el = h('aside', { class: 'drawer', id: 'drawer', role: 'complementary', 'aria-label': 'Side panel', hidden: true },
      h('div', { class: 'drawer-head' }, h('h2', { id: 'drawer-title' }), h('button', { class: 'btn icon ghost', 'aria-label': 'Close panel', onClick: () => this.close() }, icon('close'))),
