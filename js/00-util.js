...
function setKids(el, ...kids) { el.textContent = ''; el.append(frag(...kids)); return el; } // textContent is safe even if a blur handler already moved a child
...
/* ===== SVG icon pack (assets/ui) — drawn as a mask so every icon takes the current text colour ===== */
const assetUrl = (path) => new URL(path, document.baseURI).href;
/* Icons are inlined (ICONS, generated from the asset pack): they follow the text colour and need no extra request. */
function icon(name, cls = '') {
  const def = (typeof ICONS !== 'undefined' && ICONS[name]) || null;
  const el = h('i', { class: 'ic ' + cls, 'aria-hidden': 'true' });
