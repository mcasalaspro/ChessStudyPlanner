...
const Panel = {
  ...
  editSession(id, { closing = false } = {}) {
    ...
    const build = () => {
      ...
      const tm = sessionTimes(s); const live = !s.ended_at; const locked = sessionLocked(s);
      ...
      const dateIn = h('input', { type: 'date', value: ymd(new Date(tm.start)), disabled: locked });
      const startIn = h('input', { type: 'time', value: hm(tm.start), step: 60, disabled: locked });
      const endIn = h('input', { type: 'time', value: hm(tm.end), step: 60, disabled: live || locked });
      const durIn = h('input', { type: 'number', min: 1, max: 960, value: Math.round(tm.gross / MIN), disabled: live || locked });
      ...
      const applyTimes = (anchor) => {
        ...
        if (errs.length) {
          // the requested amount of time matters more than the exact minute: push the neighbours if there is room
          const moves = planPush(c.start, c.end, id, c.end - tm.end >= 0 ? 1 : -1);
          if (moves && moves.length) {
            applyBatch([{ id, patch }, ...pushMoves(moves)]);
            errBox.hidden = true; toastUndo(`Block updated · ${moves.length} neighbour${moves.length > 1 ? 's' : ''} shifted`); build(); Calendar.offerMerge(id); return;
          }
          showErrors(errs); build(); return;
        }
        errBox.hidden = true; updateSession(id, patch); toastUndo(closing ? 'Adjusted' : 'Block updated'); Calendar.offerMerge(id);
      };
      ...
      const cand = live || locked ? null : mergeCandidate(id, 90);
      const mergeRow = cand ? (() => { const o = cand.other; const otm = sessionTimes(o);
        return h('div', { class: 'merge-row' }, h('span', { class: 'grow small' }, `Another ${themeName(o.theme)} block ${fmtRange(otm.start, otm.end)}${cand.gap ? ` · ${fmtHM(cand.gap / MIN)} apart` : ' · right next to it'}`),
          h('button', { class: 'btn sm', onClick: () => { const m = mergeSessions(id, o.id); if (m) { toastUndo('Blocks merged'); Panel.editSession(m.id); } } }, '⇔ Merge')); })() : null;
      const lockNote = locked ? h('p', { class: 'hint warn' }, '🔒 This day is closed. Times and deletion are protected — notes and rating can still be edited.') : null;
      const presets = live || locked ? null : h('div', { class: 'row preset-row' }, h('span', { class: 'muted small' }, 'Set length:'),
