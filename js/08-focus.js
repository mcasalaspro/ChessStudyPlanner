...
const Focus = {
  ...
  open() {
    ...
    const center = h('div', { class: 'fz-center' }, missionBox, h('div', { class: 'fz-frozen num', id: 'fz-frozen' }), h('div', { class: 'fz-main num', id: 'fz-main' }, '00:00:00'), h('div', { class: 'fz-sub', id: 'fz-sub' }),
      ...
      h('div', { class: 'fz-break', id: 'fz-break' }));
    const q = quoteOfDay();
    ...
    el.append(h('div', { class: 'fz-hint' }, 'Space break/resume · N note · M theme · E stop · Esc exit'), center, quote, noteBox, bar);
    ...
    this.offs = [() => document.removeEventListener('keydown', onKey, true), () => document.removeEventListener('visibilitychange', onVis), () => document.removeEventListener('fullscreenchange', onFs), () => window.removeEventListener('resize', onResize), on('tick', () => { this.updateDigits(); this.renderPlan(); }), on('change', () => { this.renderControls(); this.renderPlan(); this.renderBreakButton(); })];
    this.t0 = performance.now(); this.resize(); this.startAnim(); this.wake(); this.updateDigits(); this.renderPlan(); this.renderBreakButton();
  },
  close(exitFs = true) {
    if (!this.el) return;
  ...
  /* One large control: start a break, or come back from it. */
  renderBreakButton() {
    const box = $('#fz-break', this.el); if (!box) return;
    const r = runningSession(); const st = Timer.status(); const onBreak = !!Timer.breakLeft();
    if (!r) { setKids(box); return; }
    if (st === 'paused') {
      setKids(box, h('button', { class: 'btn primary fz-big', onClick: () => (onBreak ? Timer.endBreak() : Timer.resume()) }, '▶ RESUME STUDY'),
        h('div', { class: 'fz-break-note' }, onBreak ? 'Guided break — it comes back on its own' : 'On a break · the clock is paused'));
    } else {
      setKids(box, h('button', { class: 'btn fz-big', onClick: () => Timer.pause() }, '❚❚ START BREAK'),
        h('div', { class: 'fz-break-note' }, 'Press when you step away — the time stops counting'));
    }
  },
  renderControls() {
    if (!this.el) return;
  ...
  renderPlan() {
    ...
    const rFree = runningSession();
    // a free run has no plan to show — that is the whole point of it
    if (rFree && rFree.meta?.free) { track.hidden = true; legend.textContent = 'Open-ended session · you decide when to break'; return; }
    track.hidden = false;
