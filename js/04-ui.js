...
function openModal({ title, body, footer, cls = '', onClose }) {
  ...
  const modal = h('div', { class: 'modal ' + cls }, title ? h('div', { class: 'modal-head' }, h('h2', null, title), h('button', { class: 'btn icon ghost', 'aria-label': 'Close', onClick: () => close() }, '✕')) : null, body, footer ? h('div', { class: 'modal-foot' }, footer) : null);
  root.append(modal);
  let closed = false;
...
function confirmDialog(message, { title, okLabel = 'Confirm', cancelLabel = 'Cancel', danger = false, twice = null } = {}) {
  return new Promise((resolve) => {
    let step = 0; const msgEl = h('p', null, message);
    const ok = h('button', { class: 'btn ' + (danger ? 'danger' : 'primary'), onClick: () => { if (twice && step === 0) { step = 1; msgEl.textContent = twice; ok.textContent = 'Confirm'; return; } m.close(true); } }, okLabel);
...
function toastUndo(msg) { toast(msg, { action: 'Undo', duration: 8000, onAction: () => { if (undoLast()) toast('Undone'); } }); }
...
function themeSelect(value, { allowNone = false } = {}) {
  const sel = h('select', null, allowNone ? h('option', { value: '' }, 'No theme') : null, ...themes().map((th) => h('option', { value: th.id, selected: th.id === value }, th.name)));
  return sel;
}
...
function noteIcon(cls = '') { return h('span', { class: 'note-ic ' + cls, title: 'Has a note', 'aria-label': 'Has a note', html: '<svg viewBox="0 0 16 16" width="11" height="11" aria-hidden="true"><path d="M2 2.5A1.5 1.5 0 0 1 3.5 1h9A1.5 1.5 0 0 1 14 2.5v7A1.5 1.5 0 0 1 12.5 11H6l-3.2 2.6c-.4.3-.8 0-.8-.5V11A1.5 1.5 0 0 1 2 9.5z" fill="currentColor"/></svg>' }); }

/* Note editor with write/preview + counter (max 20 000) */
function noteEditor(initial = '', { placeholder = 'Block note: what you studied, ideas, to-dos… (**bold**, *italic*, - lists)', rows = 4, onChange } = {}) {
  const ta = h('textarea', { rows, placeholder, maxlength: 20000, 'aria-label': 'Note' }); ta.value = initial || '';
  ...
  const seg = segmented([['write', 'Write'], ['preview', 'Preview']], 'write', (v) => { const p = v === 'preview'; preview.hidden = !p; ta.hidden = p; if (p) preview.innerHTML = renderMd(ta.value) || '<p class="muted">Nothing to preview.</p>'; }, 'sm');
...
const Drawer = {
  el: null, current: null,
    ...
    this.el = h('aside', { class: 'drawer', id: 'drawer', role: 'complementary', 'aria-label': 'Side panel', hidden: true },
      h('div', { class: 'drawer-head' }, h('h2', { id: 'drawer-title' }), h('button', { class: 'btn icon ghost', 'aria-label': 'Close panel', onClick: () => this.close() }, '✕')),
